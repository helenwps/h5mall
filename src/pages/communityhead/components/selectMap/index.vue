<template>
  <van-popup v-model="visible" class="ch-selectmap">
    <div class="member-content" v-if="visible">
      <h3 class="member-content-title">
        <van-icon class="arrow-left" name="arrow-left" @click="onClose" />
        输入详细地址
      </h3>
      <div class="topbar">
        <div class="search-box">
          <div class="search-wrap">
            ...
            <input
              ...
              placeholder=""
              class="van-field__control--center"
            /> -->
          </div>
        </div>
      ...
      <div v-if="spotList && spotList.length > 0">
        <!-- <van-list
          v-model="loading"
          :finished="finished"
          finished-text="没有更多了"
        ...
        >
          <van-radio :name="item" v-for="(item, index) in spotList" :key="index">
            <div class="address-list-item">
              <div class="address-name">
                <span class="address-detail">{{ item.name }}</span>
                <span>{{ item.address }}</span>
              </div>
              <span class="address-distance" v-if="item.distance !== null"
      ...
      <div v-else class="empty-phrase">暂无数据，建议换个关键字查询</div>
    </div>
  </van-popup>
</template>
<script>
